...
namespace EmployeeManagmentSystem.Blazor.Forms
{
    public class DeptStateValidator : OwningComponentBase<EmpManagerDbContext>
    {
        public EmpManagerDbContext Context => Service;
        ...
        private void Validate(Person? model, ValidationMessageStore store)
        {
            if(model?.DepartmentId == DepartmentId && LocationStates != null && 
                CurrentEditContext != null &&(!LocationStates.ContainsKey(model.LocationId) || LocationStates[model.LocationId] != State))
            {
